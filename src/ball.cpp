...
BallUserData Ball::create(entt::registry& registry, b2WorldId worldId, BallUserData* userDataContainer, int index) {
    ...
    registry.emplace<BallId>(entity);
    registry.emplace<BallFrame>(entity, Rectangle({ 0, 0, BALL_SIZE, BALL_SIZE }), ballTexture, 0, 0, 8);
    
    // Link entity with body definition and create body
    bodyDef.userData = userDataContainer + index;
    b2BodyId bodyId = b2CreateBody( worldId, &bodyDef );
    registry.emplace<BallCollision>(entity, bodyId, 0.0f);
...
void Ball::render(const BallCollision& col, const BallFrame& tex) const {
    b2Vec2 pos = b2Body_GetWorldPoint(col.bodyId, b2Vec2({ 0, 0 }));
    Vector2 p = screen.b2Vec2ToVector2(pos);
    
    DrawTexturePro(tex.texture, tex.frame, { p.x, p.y, BALL_SIZE, BALL_SIZE }, ballExtent, col.angle, WHITE);
}

void Ball::update(BallCollision& col, BallFrame& tex) {
    b2Vec2 vel = b2Body_GetLinearVelocity(col.bodyId);
    float speed = fabsf(b2Length(vel))*10;
    ...
    if (tex.frameCounter >= 60/speed) {
        ...
        if (tex.frameIndex > BALL_FRAMES-1) tex.frameIndex = 0;
